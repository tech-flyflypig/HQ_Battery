...
void BMS1Processor::configure()
{
    // 配置处理器，例如初始化batteryData的默认值
    batteryData = BMS_1();
}

void BMS1Processor::processSerialData(const QByteArray &data)
{
...
QByteArray BMS1Processor::getQueryCommand() const
{
    return createModbusCommand(SLAVE_ADDR, READ_HOLDINGS,
                             DATA_START_REG, DATA_REG_COUNT);
}

QByteArray BMS1Processor::getControlCommand(uint16_t reg, uint16_t value) const
{
...
bool BMS1Processor::isValidControlValue(uint16_t reg, uint16_t value) const
{
    // 检查寄存器地址是否有效
...
QByteArray BMS1Processor::createModbusCommand(uint8_t slave, uint8_t func, 
...
quint16 BMS1Processor::calculateCRC16(const QByteArray& data) const
{
