...
void BMS1Interface::configure()
{
    // 配置处理器，例如初始化batteryData的默认值
    batteryData = BMS_1();
}

void BMS1Interface::processSerialData(const QByteArray &data)
{
...
QByteArray BMS1Interface::getQueryCommand() const
{
    return createModbusCommand(SLAVE_ADDR, READ_HOLDINGS,
                             DATA_START_REG, DATA_REG_COUNT);
}

QByteArray BMS1Interface::getControlCommand(uint16_t reg, uint16_t value) const
{
    if (!isValidControlValue(reg, value)) {
        return QByteArray();
    }
    return createModbusCommand(SLAVE_ADDR, WRITE_MULTIPLE, reg, value);
}

bool BMS1Interface::isValidControlValue(uint16_t reg, uint16_t value) const
{
    // 检查寄存器地址是否有效
...
QByteArray BMS1Interface::createModbusCommand(uint8_t slave, uint8_t func, 
...
quint16 BMS1Interface::calculateCRC16(const QByteArray& data) const
{
